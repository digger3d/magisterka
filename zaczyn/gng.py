...
for edge in gg.edges:
    G.add_edge(gg.nodes.index(edge.get_ends()[0]), gg.nodes.index(edge.get_ends()[1]))
...
def get_weight(sl1, sl2):
    return 1/sum((np.array(sl1) - np.array(sl2))**2) / sum((np.array(sl1) + np.array(sl2))**2)
...
for edge in gg.edges:
    ...
    pos1 = node1.data.pos
    pos2 = node2.data.pos
    plt.plot([pca.transform(pos1)[:, 0], pca.transform(pos2)[:, 0]], [pca.transform(pos1)[:, 1], pca.transform(pos2)[:, 1]], 'b-', lw=weight)